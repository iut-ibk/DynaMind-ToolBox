#include "gdaladdcomponentviewcontainer.h"
#include "ogrsf_frmts.h"

DM_DECLARE_NODE_NAME(GDALAddComponentViewContainer, Modules)

GDALAddComponentViewContainer::GDALAddComponentViewContainer()
{
	GDALModule = true;
	elements = 100000;
	this->addParameter("elements", DM::INT, &elements);
	append = false;
	this->addParameter("append", DM::BOOL, &append);
}

void GDALAddComponentViewContainer::run()
{
	int counter = 0;
	for (int i = 0; i < elements; i++) {
		OGRFeature * f = this->components.createFeature();
		f->SetField("persons", "that is me");
		OGRPoint pt;
		pt.setX( i );
		pt.setY( i );

		f->SetGeometry(&pt);
		if (counter == 100000) {
			counter = 0;
		}
		counter++;
	}
}

void GDALAddComponentViewContainer::init()
{
	if (!append) {
		components = DM::ViewContainer("component", DM::NODE, DM::WRITE);
	}
	else {
		components = DM::ViewContainer("component", DM::NODE, DM::MODIFY);
	}
	components.addAttribute("persons",DM::Attribute::STRING, DM::WRITE);

	std::vector<DM::ViewContainer * > datastream;
	datastream.push_back(&components);
	this->registerViewContainers(datastream);
}
