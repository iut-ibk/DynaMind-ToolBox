...
void DM_Strahler::init()
{
	...
	network.addAttribute("strahler_number",  DM::Attribute::INT, DM::WRITE);
...
void DM_Strahler::run()
{
	...
	for (std::map<long, long >::const_iterator it = node_con_counter.begin();
		 ...
		 it++) {
		//DM::Logger(DM::Error) << "Start Node " << it->second;
		if (it->second != 1)
			continue;
		// Get starting edge with inital strahler number 1
		getNext(it->first, 1);
	}
	// network.syncAlteredFeatures();

	// Write Strahler
	network.resetReading();
	// [end_id] -----> [start_id]
	while (f = network.getNextFeature()) {
		f->SetField("strahler_number", strahler_id[f->GetFID()]);
	}
