...
#include "ui_guiconditionalloopgroup.h"
#include "conditionalloopgroup.h"
#include <QInputDialog>
#include <QMessageBox>

GUIConditionalLoopGroup::GUIConditionalLoopGroup(ConditionalLoopGroup * m, QWidget *parent):
	QDialog(parent),
	ui(new Ui::GUIConditionalLoopGroup)
{
	ui->setupUi(this);

	this->m = m;
	
	ui->conditionEdit->setText(QString::fromStdString(m->conditionString));
...
GUIConditionalLoopGroup::~GUIConditionalLoopGroup()
{
	delete ui;
}

void GUIConditionalLoopGroup::insertStreamEntry(std::string name, bool write)
{
	if(write)
...
void GUIConditionalLoopGroup::on_addReadStream_clicked()
{
...
void GUIConditionalLoopGroup::on_addWriteStream_clicked()
{
...
void GUIConditionalLoopGroup::on_removeStream_clicked()
{
...
void GUIConditionalLoopGroup::accept() 
{
	m->conditionString = ui->conditionEdit->text().toStdString();
	QDialog::accept();
}
