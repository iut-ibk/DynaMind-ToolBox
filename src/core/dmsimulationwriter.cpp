    ...
    foreach(Module * m, modules) {
        ...
        for (std::map<std::string, int>::iterator it = parameterList.begin(); it != parameterList.end(); ++it) {
            if (it->second < DM::USER_DEFINED_INPUT) {
                QString parameterName(QString::fromStdString(it->first));
                QString parameterValue(QString::fromStdString(m->getParameterAsString(it->first)));

                if (it->second == DM::FILENAME ) {
                    QDir simulation_file_dir  = QFileInfo(QString::fromStdString(filename)).absoluteDir();
                    QString relative_file = simulation_file_dir.relativeFilePath(parameterValue);
                    Logger(Debug) << "Original " << parameterValue;
                    Logger(Debug) << "Filename " << relative_file.toStdString();
                    parameterValue = relative_file;

                }

                out <<  "\t\t\t<parameter name=\"" << parameterName <<"\"" << ">"
                     << "\n" "\t\t\t\t<![CDATA["
                     <<  parameterValue
                      << "]]>" << "\n"
                      << "\t\t\t</parameter>\n";
