...
void Geometry::VectorDataToRasterData() {
    //std::cout <<  "Run Vector to RasterData" << std::endl;
}


Segment_list_2 Geometry::Snap_Rounding_2D(const VectorData & vec, std::string id, float Tolerance)  {

    //std::cout << "Snap Rounding 2D" << std::endl;

    Segment_list_2 seg_list;
    Polyline_list_2 output_list;

    seg_list = Geometry::VecToSegment2D(vec, id);

    // Generate an iterated snap-rounding
    CGAL::snap_rounding_2<Traits,Segment_list_2::const_iterator,Polyline_list_2>
            (seg_list.begin(), seg_list.end(), output_list, Tolerance, false, false, 1);
...
Segment_list_2 Geometry::VecToSegment2D(const VectorData & vec, std::string id) {
    Segment_list_2 seg_list;
    std::vector<std::string> names = VectorDataHelper::findElementsWithIdentifier(id, vec.getPointsNames());

    foreach (std::string id, names) {
        std::vector<Point> points = vec.getPoints(id);
        std::vector<Edge> edges = vec.getEdges(id);
        for (std::vector<Edge>::const_iterator it = edges.begin(); it != edges.end(); ++it) {
            Segment_2 seg(Point_2(points[it->id1].x, points[it->id1].y), Point_2(points[it->id2].x, points[it->id2].y));
...
VectorData Geometry::Segment2DToVec(Segment_list_2 seg_list, std::string id) {
    VectorData vec;
    std::vector<Point> vp;
    std::vector<Edge> ve;
    ...
    for ( Segment_list_2::const_iterator seg = seg_list.begin(); seg != seg_list.end(); ++seg) {
        ...
        vp.push_back(Point (x1, y1, 0));
        vp.push_back(Point (x2, y2, 0));
        ve.push_back(Edge(vp.size()-2, vp.size()-1));
    }
...
VectorData Geometry::ShapeFinder(const VectorData & vec, std::string id, std::string return_id, bool withSnap_Rounding, float Tolerance, bool RemoveLines)  {
    ...
    if (withSnap_Rounding == true) {
        segments = Geometry::Snap_Rounding_2D(vec, id, Tolerance);
    } else {
        segments = Geometry::VecToSegment2D(vec, id);
    }
    ...
    if (RemoveLines == true){
        ...
        do {
            ...
            vibens::Logger(vibens::Debug)<< "Counter " << removecounter;
    ...
    for (fit = arr.faces_begin(); fit != arr.faces_end(); ++fit) {
        if (! fit->is_unbounded()) {
            ...
            next++;
            ////std::cout << hec->curve().target() - next->curve().source() << std::endl;
            ...
            do{
                ...
                bool source = false;
                bool target = false;
                for ( int i = 0; i < ressults_P2.size(); i++) {
            ...
            ve.push_back(Edge(vp.size()-1, 0));
            vf.push_back(f);
            vd.push_back( Geometry::CalculateBoundingBox(vp) );


            double Area = VectorDataHelper::calculateArea(f, vp);

            //extrudeFace(vp, vf, 6);
            if (Area > 0) {
                std::stringstream name;
                name << return_id << k++;
                return_vec.setPoints(name.str(), vp);
                return_vec.setEdges(name.str(), ve);
                return_vec.setFaces(name.str(), vf);
                return_vec.setDoubleAttributes(name.str(), vd);
            }
    ...
    vibens::Logger(vibens::Debug)<< "Number of extracted Faces " << k;
    std::vector<double> db;
    db.push_back(k);
    std::stringstream s;
    s << "numberOf" << return_id;
    return_vec.setDoubleAttributes(s.str(), db);
...
VectorData Geometry::createRaster(std::vector<Point> & points, double width, double height) {
    ...
    /*BOOST_FOREACH(Point p, poly_new) {
            poly_s.push_back(Point_2(p.x, p.y));            
        }			*/
